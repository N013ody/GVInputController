...
```text
GVInputController/
  GVInput.hpp                  单头文件库，GVInput 枚举、调用、安装封装
  GVInputController.cpp         示例命令行程序
  GVInputResources.rc           可选：把驱动包嵌入 exe 的资源脚本
  GVInputController.sln         Visual Studio 解决方案
  GVInputController.vcxproj     Visual Studio C++ 项目
  drivers/gvinput/              可选：本地驱动包，不建议提交公开仓库
    gvinput.inf
    gvinput.cat
    gvinput.sys
    gvinputmf.inf
...
公开仓库建议只提交 `GVInput.hpp` 和必要文档，不提交 `.sys`、`.cat`、`.dll`、`.vs`、`x64`、`Debug` 等二进制或构建产物。
